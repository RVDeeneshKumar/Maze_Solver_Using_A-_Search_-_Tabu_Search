...
def main(n, speed):
    m = maze(n, n)
    m.CreateMaze(loadMaze='maze--2022-05-23--17-57-41.csv')

    searchPath, aPath, fwdPath = aStar(m)
    a = agent(m, n, n, footprints=True, color=COLOR.blue, filled=True)
    b = agent(m, 1, 1, footprints=True, color=COLOR.red, filled=True, goal=(m.rows, m.cols))
    c = agent(m, n, n, footprints=True, color=COLOR.red, filled=True)

    m.tracePath({a: searchPath}, delay=speed)
    m.tracePath({b: aPath}, delay=speed)
    m.tracePath({c: fwdPath}, delay=speed-1)
